...
pdf("../figures/concept_figures/weightedData.pdf",width=6,height=5)
newpoint <- 7030
...
plot(trainDat_weights[,"A"],
     trainDat_weights[,"B"],pch=16,
     xlim=c(min(newdat[,"A"]),max(newdat[,"A"])),
     ylim=c(min(newdat[,"B"]),max(newdat[,"B"])),axes=F,xlab="",ylab="")
Axis(side = 3, x = trainDat_weights[,"A"])
Axis(side = 4, x = trainDat_weights[,"B"])
mtext("Predictor A (scaled and weighted)", side = 3,line=3)
mtext("Predictor B (scaled and weighted)", side = 4,line=3)



#par(fig=c(0.26,0.49,0.53,0.98), new=TRUE)
#par(mgp=c(1,0.4,0))
par(fig=c(0.5,0.98,0.54,0.98), new=TRUE)
#par(mgp=c(1,0.4,0))
barplot(unlist(t(data.frame(varImp(model,scale=F)$importance))),horiz=T,xlab="",cex.axis=0.75,cex.names=0.75)
title(xlab="Importance", line=1.8,cex.lab=0.75,adj=0)
