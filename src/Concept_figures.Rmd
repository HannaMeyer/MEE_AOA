...
#par(fig=c(0,1,0,1), new=TRUE,mar=c(4,4,4,4))
...
#mtext("Predictor B (scaled)", side = 4,line=3)
...
text(trainDat_weights[whichIsMin,"A"],trainDat_weights[whichIsMin,"B"],paste0("dist=",round(mindist,2),", \nDI= ",round(mindist/attributes(newdat_aoa)$aoa_stats$Mean_train,2)),pos=2,cex=0.85)
legend("topleft",pch=c(rep(16,3),1),col=c(cols_fold,"red"),legend=c("Fold 1","Fold 2","Fold 3","Example"),bty="n")
...
#par(fig=c(0.79,1,0.34,1), new=TRUE)
par(fig=c(0.57,0.68,0.44,1), new=TRUE)

par(mgp=c(1,0.49,0))

 boxplot(attributes(newdat_aoa)$TrainDI$DI,cex.axis=0.6,cex.lab=0.5,ylab="",yaxt="n")
 axis(side = 2, at = c(0,0.2,0.4,0.6),cex.axis=0.7)
    abline(boxplot.stats(attributes(newdat_aoa)$TrainDI$DI)$stats[5],0,col=alpha("blue",0.5),lwd=2)
    title(ylab="DI of training data", line=1.5,cex.lab=0.6)
    text(0.7,boxplot.stats(attributes(newdat_aoa)$TrainDI$DI)$stats[5],
...
pdf("../figures/concept_figures/threshold_thoughts.pdf")
 boxplot(attributes(newdat_aoa)$TrainDI$DI,ylab="DI of train data")
 abline(attributes(newdat_aoa)$aoa_stats$threshold_stats[5],0,col="red")
 abline(attributes(newdat_aoa)$aoa_stats$threshold_stats[6],0,col="red",lty=2)
  ...
  abline(attributes(newdat_aoa)$aoa_stats$threshold,0,col="blue")
  legend("bottomright",lty=c(1,2,3,1),col=c("red","red","red","blue"),legend=c("0.95 quantile","0.99 quantile","1.00 quantile","1.5*IQR"),bty="n")
 dev.off()
...
## Why can't we use the distance to all data points?
