...
if(nrow(model$trainingData>=250)){
  window.size <- 50 # clustered has more data so a larger window size is possible
}else{
  window.size <- 10
}
...
legend("topright",lty=c(NA,NA,2),lwd=c(NA,NA,2),pch=c(1,1,NA),col=c("black","red","blue"),
  ...
  lines(seq(0,max(recl$DI),0.01),
        predict(attributes(AOA_new)$calib$model,data.frame("DI"=seq(0, max(recl$DI),0.01))),lwd=2,lty=2,col="blue")
...
print(summary(attributes(AOA_new)$calib$model))

# model using multiple-CV
print(summary(attributes(AOA_new_MCV)$calib$model))
```
