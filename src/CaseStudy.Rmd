...
* Default Case study with random data: npoints=50,meansPCA= c(3, -1), sdPCA=c(2, 2),simulateResponse=c("bio2","bio5","bio10", "bio13","bio14","bio19"),studyarea=c(-15, 65, 30, 75), seed=10

* Case Study with clustered data: npoints=500,nlusters=50,
...
AOA <- aoa(trainDat,predictors, variables = names(predictors),model=model,cl=cl,folds=folds)
#without weighting:
AOA_noWeights <- aoa(trainDat,predictors, variables = names(predictors),cl=cl,folds=folds)
...
#DI with weights:
summary(lm(values(truediff)~values(AOA$DI)))$r.squared
#DI no weights:
summary(lm(values(truediff)~values(AOA_noWeights$DI)))$r.squared
