...
dat2 <- data.frame("DI"=attributes(AOA)$TrainDI$DI,
                   "error"=absError)


png("../figures/hexbin.png", width=12, height=10,units="cm",res=300)
p1 <- ggplot(dat_all, aes(value,true_abs_error)) +
  ...
  #stat_smooth(data=dat2,aes(DI,error),method="lm")
...
newAOA <- update_aoa(AOA,model,proficiency = 0.05)
cairo_pdf("../figures/aoathresholds.pdf", width=12, height=5)
p2 <- attributes(newAOA)$update_aoa+xlab("Dissimilarity Index")+ylab("True absolute error")
grid.arrange(p1, p2, ncol = 2)

invisible(dev.off())

png("../figures/newaoa.png",width=10, height=10,units = "cm",res=300)
sp::spplot(prediction, col.regions=viridis(100))+sp::spplot(newAOA$AOA,col.regions=c("violetred","transparent"))
invisible(dev.off())

rmse(values(prediction)[values(AOA$AOA)==1],values(response)[values(AOA$AOA)==1],na.rm=T)
rmse(values(prediction)[values(newAOA$AOA)==1],values(response)[values(newAOA$AOA)==1],na.rm=T)
